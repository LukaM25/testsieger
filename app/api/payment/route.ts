...
export async function POST(req: Request) {
  ...
  if (!priceId) {
    return NextResponse.json(
      {
        error: `Plan nicht konfiguriert (${selectedPlan}). Bitte STRIPE_PRICE_${selectedPlan} setzen.`,
      },
      { status: 500 }
    );
  }
  ...
  let checkoutMode: "payment" | "subscription" =
    selectedPlan === "LIFETIME" ? "payment" : "subscription";
    ...
    stripeSession = await stripe.checkout.sessions.create(
      {
        ...
        success_url: `${baseUrl}/dashboard?checkout=success&productId=${encodeURIComponent(
          product.id
        )}`,
        cancel_url: `${baseUrl}/pakete?checkout=cancel&productId=${encodeURIComponent(
          product.id
  ...
  await prisma.order.create({
    data: {
      ...
      productId: product.id,
      plan: selectedPlan,
      priceCents: priceAmountCents ?? 0,
      stripeSessionId: stripeSession.id,
      stripePriceId: priceId,
      stripeCurrency: priceCurrency ?? "eur",
      stripeUnitAmountCents: priceAmountCents ?? 0,
      stripeSubtotalCents: priceAmountCents ?? 0,
